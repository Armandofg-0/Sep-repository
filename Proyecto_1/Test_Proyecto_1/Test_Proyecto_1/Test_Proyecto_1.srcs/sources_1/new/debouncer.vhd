...
entity Debouncing_Button_VHDL is
    generic (
        CLK_FREQ_HZ      : integer := 100_000_000; -- Frecuencia del clk de la FPGA
        DEBOUNCE_TIME_MS : integer := 20           -- Tiempo de debounce en ms
    );
    port(
        clk              : in  std_logic;
        button           : in  std_logic;
        debounced_pulse  : out std_logic  -- Pulso único por pulsación
    );
...
architecture Behavioral of Debouncing_Button_VHDL is
    ...
    -- Sincronización para evitar metastabilidad
...
begin
    ...
    -----------------------------------------------------------------
    -- Sincronizar el botón con el reloj principal
    -----------------------------------------------------------------
    process(clk)
    begin
    ...
    -----------------------------------------------------------------
    -- Pulso único en flanco de subida limpio
    -----------------------------------------------------------------
    process(clk)
    begin
        if rising_edge(clk) then
